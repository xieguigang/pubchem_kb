    ...
    (function (EventHandles) {
        ...
        */
        ...
        function hookEventHandles(app) {
            var elements = findAllElement();
            var type = TypeScript.Reflection.$typeof(app);
            var _loop_1 = function (methodName) {
                if (elements.indexOf(methodName) > -1) {
                    var arguments_1 = parseFunctionArgumentNames(app[methodName]);
                    if (arguments_1.length == 0 || arguments_1.length == 2) {
                        $ts("#" + methodName).onclick = function (handler, evt) {
