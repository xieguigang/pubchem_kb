...
CREATE TABLE IF NOT EXISTS `admin` (
  ...
  `flag` INT NOT NULL DEFAULT 0 COMMENT '对于离职员工，将flag设为1',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
COMMENT = '系统管理员以及收银员';
...
CREATE TABLE IF NOT EXISTS `waterflow` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `transaction_id` VARCHAR(45) NOT NULL COMMENT '交易编号',
  `goods` MEDIUMTEXT NOT NULL COMMENT '{goods.id:count} 键值对的json字符串',
  `time` DATETIME NOT NULL,
  `money` DOUBLE NOT NULL COMMENT '交易的总金额',
  `vip_balance` DOUBLE NOT NULL DEFAULT 0 COMMENT '如果是会员的话，通过会员的账户余额所交易的金额部分的记录',
  ...
  `operator` INT NOT NULL,
  `note` MEDIUMTEXT NULL,
  `flag` TINYINT(1) NULL DEFAULT 0 COMMENT '交易是否被撤销？',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `VIP_waterflow` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `vip` INT NOT NULL,
  `balance` DOUBLE NOT NULL COMMENT '会员账号的会员充值余额的额度变化',
  `waterflow_id` INT NOT NULL COMMENT '-1 表示充值，其他数值表示消费购买以及退款的交易单编号',
  `type` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '0 正常交易\n1 充值\n-1 退款',
  `time` DATETIME NOT NULL,
  `operator` INT NOT NULL,
  `note` MEDIUMTEXT NULL,
  PRIMARY KEY (`id`))
...
SHOW WARNINGS;
CREATE UNIQUE INDEX `id_UNIQUE` ON `page_views` (`id` ASC);
